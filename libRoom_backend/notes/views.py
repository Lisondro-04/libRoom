...
class NotesListView(APIView):
    ...
    def post(self, request):
        data = request.data
        note_id = utils.get_next_id()
        now = datetime.utcnow().isoformat()

        tag = data.get('linked_to', '')
        tag_type, target_id = tag.split("|") if "|" in tag else (tag, None)

        wc = utils.save_note_md(note_id, data['title'], tag, data['content'])
...
class NoteDetailView(APIView):
    ...
    def get(self, request, note_id):
        content = utils.load_note_md(note_id)
        if content is None:
        ...
        if not note_meta:
            return Response({"error": "Nota no encontrada"}, status=404)
        # Retorna datos completos, puedes ajustar campos aquí
        response_data = {
            "id": note_meta["id"],
    ...
    def delete(self, request, note_id):
        ...
        utils.save_index(index)
        # Borrar archivo .md
        file_path = utils.BASE_DIR / f"{note_id}.md"
        if file_path.exists():
            file_path.unlink()
